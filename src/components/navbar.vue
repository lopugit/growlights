<template lang='pug'>
  q-toolbar.fixed-top.q-nav-toolbar.navbar(
    ...
    q-btn(
      color="primary"
      @click="$store.commit('thing', { path: 'leftSidebar', val: true}) ; $store.dispatch('login', {provider: 'alopu'})"
    ...
    q-btn(
      ...
      q-chip(
        v-if="gosmart($store, 'state.alopu.entity.alopu.carts.0.products', []).length"
        floating
        color="red"
        small
      ).q-circle.cart-chip {{ gosmart($store, 'state.alopu.entity.alopu.carts.0.products', []).length }}
